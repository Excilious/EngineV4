...
Skybox::Skybox(int WindowWidth,int WindowHeight,float SkyNear,float SkyFar,Parser *InstanceParser,float FieldOfView)
{
...
void Skybox::RenderSkybox(glm::vec3 Position,glm::vec3 Orientation,glm::vec3 UpPosition,Shader* SkyboxShader)
{
    ...
    Projection  = glm::perspective(glm::radians(this->FieldOfView),(float)this->Width/this->Height,this->Near,this->Far);
...
void Skybox::Destroy()
{
    for (int Index = 0; Index < this->ImageBuffers.size(); Index++) {free(this->ImageBuffers[Index]);}
}